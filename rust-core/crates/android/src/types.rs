...
pub fn to_jstring(env: &mut JNIEnv, s: &str) -> jni::objects::JString {
    env.new_string(s)
...
pub fn from_jstring(env: &mut JNIEnv, s: &JString) -> Option<String> {
    if s.is_null() {
        return None;
    }
    env.get_string(s)
...
/// 序列化结果到 JSON 字符串
pub fn to_json<T: serde::Serialize>(env: &mut JNIEnv, value: &T) -> jni::objects::JString {
